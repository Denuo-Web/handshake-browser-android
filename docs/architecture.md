...
- `MainActivity`: WebView browser shell with custom omnibox, shared HNS host policy, Service Worker HNS interception setup, security state, and navigation controls.
...
- `HnsSyncForegroundService`: Android 14+ `dataSync` foreground service that owns repeated native sync, keeps a user-visible notification active, and broadcasts sync status snapshots with explicit no-progress and per-peer failure stages to the UI.
- `HnsSyncScheduler`: single-threaded scheduler used by the foreground service to call the native sync tick and publish sync status snapshots.
